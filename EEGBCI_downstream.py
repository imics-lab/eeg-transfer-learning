...
from downstream.BCI_edf import EEGBCI_Dataset
from downstream.BCI2A_npy import BCIIV2A_Dataset
from sklearn.model_selection import KFold

from models import Supervised_EEGBCI
...
parser.add_argument('--task-number', type=int, default=1, help='task number to load for EEGBCI dataset')
...
def main():
    ...
    #Load Dataset
    dataset = EEGBCI_Dataset(task_number = args.task_number) 
        
    
    #Load model
    model = Supervised_EEGBCI()
...
#             model = nn.DataParallel(model)
    ...
    # Print fold results
    f = open("EEGBCI_results.txt", "a")
    f.write(f"Task {args.task_number}, initial layers {args.layers}\n")
    ...
    f.close()
#     writer.close()
    print(f'f"Task {args.task_number}, initial layers {args.layers} done')
